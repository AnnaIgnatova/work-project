...
import {GetUsersRequest, SearchUsersParams} from "./types";
...
export const searchUsersFx = createEffect(async (params?: SearchUsersParams) => {
  ...
  const res = await octokit.request(
    `GET /search/users?q=${searchName?.length ? `${searchName}+in%3Alogin+` : ""}${
...
export const $fetchError = restore<Error>(searchUsersFx.failData, null);

export const $usersGetStatus = combine({
  loading: searchUsersFx.pending,
  error: $fetchError,
  data: $responseUser,
});
